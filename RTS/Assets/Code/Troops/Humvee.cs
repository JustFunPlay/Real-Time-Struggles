...
public class Humvee : TroopMovement
{
    ...
    {
        canFire = false;
        if (Physics.Raycast(FirePoint.position, FirePoint.forward, out RaycastHit hit, range, hitlayer))
        {
        ...
        else
        {
            LineRenderer newLine = Instantiate(line);
            newLine.SetPosition(0, FirePoint.position);
            newLine.SetPosition(1, FirePoint.position + FirePoint.forward * range);
        }
